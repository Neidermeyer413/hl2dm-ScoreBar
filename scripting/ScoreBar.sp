...
public Action updateScores(Handle timer)
{
	if (GetConVarInt(teamPlay) == 0 && GetClientCount(true) > 1){ //Free for all, requires 2 players or more
		...
		for (new client = 1; client <= MaxClients; client++)
		{
			...
			{
				SetHudTextParamsEx(0.45, 0.03, 999.0, {255, 0, 0, 150}, {255, 0, 0, 150}, 0, 0.0, 0.0, 0.0);
				ShowHudText(client, 255, " %d", GetTeamScore(3));
				
				SetHudTextParamsEx(0.55, 0.03, 999.0, {0, 0, 255, 150}, {0, 0, 255, 150}, 0, 0.0, 0.0, 0.0);
				ShowHudText(client, 254, " %d", GetTeamScore(2));
			}
		}
	}
	return Plugin_Stop; 
